...
export class SendCodeTwoFactorUseCase implements UseCase<{username: string}, ForgotPasswordModel> {
    ...
    execute(state: { username: string }): Observable<ForgotPasswordModel> { 
        return this.forgotPasswordRepository.sendCodeTwoFactor(state)
    }
}